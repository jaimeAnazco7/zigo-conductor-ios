...
class BaseLanguage {
  ...
  /// Mensaje si el año de fabricación está fuera del rango permitido (Perú: máx. 15 años de antigüedad).
  /// Si el idioma del servidor no define la clave 395, se evita `KEY_NOT_FOUND` y se muestra un texto claro con el rango actual.
  String get carProductionYearInvalid {
    final fromServer = getContentValueFromKey(395);
    if (!fromServer.startsWith(defaultKeyNotFoundValue)) return fromServer;
    final now = DateTime.now();
    final minY = now.year - kMaxVehicleAgeYearsPeru;
    final maxY = now.year;
    return 'El año de fabricación debe estar entre $minY y $maxY (antigüedad máxima del vehículo: $kMaxVehicleAgeYearsPeru años).';
  }
}
