...
abstract class _AppStore with Store {
  ...
  Future<void> setUserProfile(String val, {bool isInitialization = false}) async {
    final normalized = normalizeDriverProfileUrl(val);
    userProfile = normalized;
    if (!isInitialization) await sharedPref.setString(USER_PROFILE_PHOTO, normalized);
  }
  ...
  Future<void> setDarkMode(bool aIsDarkMode) async {
    ...
    if (isDarkMode) {
      ...
      textSecondaryColorGlobal = neonHighlight;
  ...
  Future<void> setLanguage(String aCode, {BuildContext? context}) async {
    ...
    try {
      await initializeDateFormatting(aCode, null);
    } catch (_) {}
    try{
