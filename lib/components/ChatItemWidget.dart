...
class ChatItemWidget extends StatefulWidget {
  ...
  ChatItemWidget({this.data, required this.historyModeOnly});
...
class _ChatItemWidgetState extends State<ChatItemWidget> {
  ...
  Widget build(BuildContext context) {
      ...
      switch (messageTypes) {
        case TEXT:
          return Column(
            ...
            children: [
              Text(widget.data!.message!, style: primaryTextStyle(color: Colors.white), maxLines: null),
              SizedBox(height: 1),
              Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Text(
                    time,
                    style: primaryTextStyle(color: !widget.data!.isMe.validate() ? neonHighlight.withOpacity(0.6) : Colors.white.withOpacity(0.6), size: 10),
                  ),
                  SizedBox(height: 2),
                  widget.data!.isMe!
                      ...
                      children: [
                        Text(
                          ...
                          style: primaryTextStyle(
                            color: !widget.data!.isMe.validate() ? neonHighlight.withOpacity(0.6) : Colors.white.withOpacity(0.6),
                            size: 10,
                          ),
                        ),
                        SizedBox(height: 2),
    ...
    if (widget.data!.isMe! && widget.data!.deleted == true) return SizedBox();
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                color: widget.data!.isMe.validate() ? neonAccent : neonSurfaceCard,
                border: widget.data!.isMe.validate() ? null : Border.all(color: neonAccent.withOpacity(0.25)),
